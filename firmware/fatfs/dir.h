 ...
 */
cbm_errno_t concat_path_filename(char *path, uint16_t pathmax, const char *dir,
				 const char *name);

cbm_errno_t dummy_action(const char *dir, const char *name);	// just a dummy action for debug purposes
cbm_errno_t traverse(char *path,	// path string (may contain wildcards and path separators)
		     bool advanced_wildcards, uint16_t max_matches,	// abort if this number of matches is reached
