...
{
	IEEE_D_PORT |= IEEE_D_PINS;
	IEEE_D2_PORT |= IEEE_D2_PINS;
}

static inline void wrd(uint8_t data)
{
	IEEE_D_PORT = (IEEE_D_PIN & ~IEEE_D_PINS) | (IEEE_D_PINS & (uint8_t) ~ data);
	IEEE_D2_PORT = (IEEE_D2_PIN & ~IEEE_D2_PINS) | (IEEE_D2_PINS & ((~ data) >> IEEE_D2_SHIFT_R));
}

static inline uint8_t rdd(void)
{
	//debug_printf("D:%02x/%02x, D2:%02x/%02x\n", IEEE_D_PIN, IEEE_D_PINS, IEEE_D2_PIN, IEEE_D2_PINS);
	return (uint8_t) ~ ((IEEE_D_PINS & IEEE_D_PIN) | ((IEEE_D2_PINS & IEEE_D2_PIN) << IEEE_D2_SHIFT_R));
}
...
static inline void settx(void)
{
	...
	IEEE_D_DDR |= IEEE_D_PINS;	// Data as output (high)
	IEEE_D2_DDR |= IEEE_D2_PINS;	// Data as output (high)
}
...
static inline void setrx(void)
{
	...
	IEEE_D_DDR &= ~IEEE_D_PINS;		// Data as input
	IEEE_D2_DDR &= ~IEEE_D2_PINS;		// Data as input
