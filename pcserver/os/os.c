...
/*****************************************************************/
/*
 * from: https://github.com/videolan/vlc/blob/master/compat/poll.c
 */
...
int poll_serial(struct pollfd *fds, unsigned nfds, int timeout) {

#if 0	// just ignore that crap
	// partially taken from https://www.codeguru.com/network/serial-communication-in-windows/
	OVERLAPPED ov;
	...
	WaitCommEvent(fd, &dwEventMask, &ov);
#endif
...
{
    // windows stupidity. Serial devices need special care and don't work
    // with other file descs

    int ret;
    if ((ret=poll_serial(fds, nfds, timeout)) != 0) {
	    return ret;
    }
